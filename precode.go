...
func getWantedTask(w http.ResponseWriter, r *http.Request) {
...
func deleteTask(w http.ResponseWriter, r *http.Request) {
	...
	task, ok = tasks[id]
	if ok {
		http.Error(w, "Что-то пошло не так...", http.StatusExpectationFailed)
		return
	}
...
func main() {
	...
	// регистрируем в роутере эндпоинт `/tasks/{id}` с методом GET, для которого используется обработчик `getWantedTask`
	r.Get("/tasks/{id}", getWantedTask)
